MODULE sbc_oce
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   wndm_rel          !: MRA wind speed module at T-point (=|U10m-Uoce|)  [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   wndm_abs          !: MRA wind speed module at T-point (=|U10m-Uoce|)  [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   wind_work_u       !: MRA u surface                                    [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   wind_work_v       !: MRA v surface                                    [m/s]
   ...
   !MRA
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   sst_lp    !: lowpass sst for THF calculatuion                     [Celsius]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   ssu_lp    !: lowpass ssu for stress calculatuion                      [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   ssv_lp    !: lowpass ssv for stress calculatuion                      [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   T_at_v          !:MRA temp interpolated at v point
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   T_at_u          !:MRA temp interpolated at u point
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   S_at_v          !:MRA sal  interpolated at v point
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   S_at_u          !:MRA sal  interpolated at u point
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   v_T             !:MRA v times T_at_v                          
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   u_T             !:MRA u times T_at_u                          
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   v_S             !:MRA v times S_at_v                          
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   u_S             !:MRA u times S_at_u   
   !/MRA
   !! * Substitutions
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_oce_alloc()
      ...
      ALLOCATE( wndm_rel(jpi,jpj) , wndm_abs(jpi,jpj) ) !MRA          !
      ALLOCATE( wind_work_u(jpi,jpj) , wind_work_v(jpi,jpj) ) !MRA          !
      ALLOCATE( T_at_v(jpi,jpj,jpk) , v_T(jpi,jpj,jpk) ) !MRA          !
      ALLOCATE( T_at_u(jpi,jpj,jpk) , u_T(jpi,jpj,jpk) ) !MRA          !
      ALLOCATE( S_at_v(jpi,jpj,jpk) , v_S(jpi,jpj,jpk) ) !MRA          !
      ALLOCATE( S_at_u(jpi,jpj,jpk) , u_S(jpi,jpj,jpk) ) !MRA          !
      ...
      ALLOCATE( tprecip(jpi,jpj) , sprecip(jpi,jpj) , fr_i(jpi,jpj) ,     &
         ...
         &      atm_co2(jpi,jpj) ,                                        &
#endif
         &      ssu_m  (jpi,jpj) , sst_m(jpi,jpj) , sst_lp(jpi,jpj) ,     & !MRA
         &      ssu_lp (jpi,jpj) , ssv_lp(jpi,jpj),                       & !MRA
