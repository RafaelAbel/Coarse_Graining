MODULE sbcssr
   ...
   USE shapiro         ! used in case of ln_sssr_flt       !MMS &DRAKKAR
...
!{ DRAKKAR : local increase of restoring. eg Med Sea in ORCA025
   REAL(wp),         ALLOCATABLE,       DIMENSION(:,:) ::  erpcoef  !: multiplicating coef for local change to erp
!}
...
!MMS:
   ...
   REAL(wp)        ::   zsdif                   ! salinity difference for erp computation
!:MMS
!MMS: reintroduce nn_icedmp in namelist as nn_ssr_ice & cdmp2d
   INTEGER, PUBLIC ::   nn_sssr_ice =   1       ! damping under ice flag 
                                                ! (  0 : no damping ; 
                                                !    1: damping as ocean ; 
                                                !   >1 : enhanced damping (x icedmp) )
   !CEB
   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sf_deds   ! structure of input deds_2d (file informations, fields read)
   !/CEB
!:MMS
...
!MMS: reintroduce nn_icedmp as nn_sssr_ice & cdmp2d
      REAL(wp),DIMENSION(jpi,jpj)  ::   zcoefice   !MMS   ! local multiplicative coefficient for SSS damping under ice
      TYPE(FLD_N) ::   sn_deds               ! informations about the cdmp2d field to be read
      !{ DRAKKAR 
      ...
      NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_deds, sn_sst, sn_sss, &
                         & ln_sssr_bnd, rn_sssr_bnd, rn_sssr_ds, nn_sssr_ice
!:MMS
      !!----------------------------------------------------------------------
      !
...
!MMS: add cdmp2d
         ...
         IF( nn_sssr >= 1)   THEN 
            CALL fld_read( kt, nn_fsbc, sf_sss )   ! Read SSS data and provides it at kt
            IF (int(rn_deds) == 999)    CALL fld_read( kt, nn_fsbc, sf_deds )         
         ENDIF
!:MMS
         !
         !                                         ! ========================= !
...
!CDIR COLLAPSE
               ...
               !
!MMS:       !add cdmp2d & nn_sssr_ice
            ! ELSEIF( nn_sssr == 2 ) THEN               !* Salinity damping term (volume flux, emp and emps)
            ELSEIF( nn_sssr == 2  .AND. (int(rn_deds) /= 999) ) THEN  !* Salinity damping term (volume flux, emp and emps)
...
!CDIR COLLAPSE
               DO jj = 1, jpj
                  DO ji = 1, jpi                            
                     zsdif = ( zsss_m(ji,jj) - sf_sss(1)%fnow(ji,jj,1) )
...
!CDIR COLLAPSE
               DO jj = 1, jpj
                  DO ji = 1, jpi
                     ...
                     IF ( rn_sssr_ds /= 0. )   zsdif = SIGN( 1., zsdif ) * MIN( rn_sssr_ds, ABS(zsdif))
                     !
                     zerp = sf_deds(1)%fnow(ji,jj,1)/rday * ( 1. - 2.*rnfmsk(ji,jj) )  &      ! No damping in vicinity of river mouths
                        &        * ( zcoefice(ji,jj) )                                 &      !MMS     DRAKKAR Modification for nn_sssr_ice
                        &        * zsdif                                               &
                        &        / ( sss_m(ji,jj) + 1.e-20   )                         &
                        &        * erpcoef(ji,jj)
...
!MMS: reintroduce nn_icedmp as nn_sssr_ice & cdmp2d
      ...
      !NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_sst, sn_sss, ln_sssr_bnd, rn_sssr_bnd
      NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_deds, sn_sst, sn_sss, &
                         & ln_sssr_bnd, rn_sssr_bnd, rn_sssr_ds, nn_sssr_ice, ln_sssr_flt, &
                         & ln_sssr_msk, sn_coast, rn_dist, nn_shap_iter
!:MMS
      !!----------------------------------------------------------------------
      !
...
!MMS:{ add cdmp2d field from sn_deds
      sn_deds = FLD_N( 'deds_2d' ,    0.     ,  'deds_2d' ,  .false.   , .true.  ,   ''        , ''       , ''         )
      !{ DRAKKAR
...
!MMS: add cdmp2d from sn_deds
            IF ( int(rn_deds) == 999) THEN
                WRITE(numout,*) '      dE/dS (restoring magnitude on SST)     rn_deds     = 2D-field (sn_deds)   mm/day'
            ELSE
            WRITE(numout,*) '      dE/dS (restoring magnitude on SST)     rn_deds     = ', rn_deds, ' mm/day'
            ENDIF
!:MMS
...
!MMS:{
      ...
      !IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'sbc_ssr: unable to allocate erpcoef array' )
!:MMS}
      !
...
!{ DRAKKAR
         ...
         IF ( ln_sssr_msk ) THEN
            ...
            IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'sbc_ssr: unable to allocate distcoast array' )
...
!MMS: add cdmp2d field from sn_deds
            IF (int(rn_deds) == 999) THEN
                ALLOCATE( sf_deds(1), STAT=ierror )
                IF( ierror > 0 ) THEN
                    CALL ctl_stop( 'sbc_ssr: unable to allocate sf_deds structure' )   ;   RETURN
                ENDIF
                ALLOCATE( sf_deds(1)%fnow(jpi,jpj,1) )
                ALLOCATE( sf_deds(1)%fdta(jpi,jpj,1,2) )
                !
                ! fill sf_deds with sn_deds and control print
                CALL fld_fill( sf_deds, (/ sn_deds /), cn_dir, 'sbc_ssr', '2d deds data file', 'namsbc_ssr' )
            ENDIF
!:MMS
      !
...
! DRAKKAR { initialize erpcoef to increase erp in the med ses
           erpcoef(:,:) = 1._wp
