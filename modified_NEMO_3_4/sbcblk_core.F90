MODULE sbcblk_core
   ...
   USE shapiro         ! used in case of ln_sst_lp
   USE avg_RA          ! used in case of ln_sst_lp
   ...
   REAL(wp) ::   rn_pfac   = 1._wp     ! multiplication factor for precipitation
   REAL(wp) ::   rn_vfac               ! MRA     multiplication factor for ice/ocean velocity in the calculation of wind stress (clem)
...
!MMS: add start
   LOGICAL :: ln_rainplussnow  = .TRUE. !: logical flag if rain and snow are already added
   LOGICAL :: ln_wind_tau_rel  = .TRUE. !: logical flag if rain and snow are already added
   LOGICAL :: ln_wind_turb_rel = .TRUE. !: logical flag if rain and snow are already added
   LOGICAL  ::   ln_pfac2d   = .FALSE.          ! logical flag for 2D multiplication factor for precipitation
   LOGICAL  ::   ln_vfac2d   = .FALSE.          ! MRA logical flag for 2D multiplication factor for ocean current
   CHARACTER(len=32) :: cl_pfacfile             ! 2D multiplication factor mask filename for precipitation
   CHARACTER(len=32) :: cl_vfacfile             ! MRA 2D multiplication factor mask filename for ocean current
   REAL(wp) , ALLOCATABLE,  DIMENSION(:,:) :: rn_pfac2d  ! 2D multiplication factor for precipitation
   REAL(wp) , ALLOCATABLE,  DIMENSION(:,:) :: rn_vfac2d  ! MRA 2D multiplication factor for ocean current
   !{ DRAKKAR add JMM
   !MRA lp filter SST
   LOGICAL :: ln_sst_flt  = .TRUE.              !: logical flag if lp filtered sst is used for THF
   LOGICAL :: ln_stress_flt  = .TRUE.              !: logical flag if lp filtered surface stress is used for THF
   ...
   REAL(wp), ALLOCATABLE, DIMENSION(:,:) :: rmskkatax  !: array for katamask (T-point)
   REAL(wp), ALLOCATABLE, DIMENSION(:,:) :: rmskkatay  !: array for katamask (T-point)
   !}
!:MMS add end
...
CONTAINS
   ...
   SUBROUTINE sbc_blk_core( kt )
      ...
      INTEGER  ::   inum     !MMS logical unit for IOM
      ...
      TYPE(FLD_N) ::   sn_tdif                                 !   "                                 "
!MMS:{
      TYPE(FLD_N) ::   sn_pfac                        !MMS
      TYPE(FLD_N) ::   sn_vfac                        !MRA
...
!:MMS}
      NAMELIST/namsbc_core/ cn_dir , ln_2m  , ln_taudif, rn_pfac,           &
         ...
         &                  ln_kata, sn_kati, sn_katj ,                     &   !MMS
         &                  ln_rainplussnow, ln_pfac2d, sn_pfac,            &   !MMS
         &                  ln_wind_tau_rel, ln_wind_turb_rel,  rn_vfac,    &   !MRA
         &                  ln_vfac2d, sn_vfac, ln_sst_flt, nn_shap_iter,   &
         &                  ln_stress_flt                                      !MRA
      !!---------------------------------------------------------------------
...
!MMS:
         sn_pfac = FLD_N( 'alpha'   ,     0     ,  'alpha'    , .false.   , .true.   ,  'yearly'  , ''       , ''         ) 
         sn_vfac = FLD_N( 'alpha'   ,     0     ,  'alpha'    , .false.   , .true.   ,  'yearly'  , ''       , ''         ) 
         ln_rainplussnow = .false. 
         rn_pfac = 1._wp
         rn_vfac = 1._wp !MRA
         !
...
!:MMS
         ...
         !
!MMS:
         ! 2D multiplication factor for precipitation
         ALLOCATE( rn_pfac2d(jpi,jpj), STAT=ierror )
         IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'sbc_blk_core: unable to allocate rn_pfac2d' )
         rn_pfac2d(:,:) = 0._wp
         !
         IF ( ln_pfac2d ) THEN
            WRITE(cl_pfacfile,'(a,a)' ) TRIM( cn_dir ), TRIM( sn_pfac%clname )
            CALL iom_open ( cl_pfacfile, inum )                                       ! open file
            CALL iom_get  ( inum, jpdom_data, sn_pfac%clvar, rn_pfac2d )             ! read the pfac array
            CALL iom_close( inum )
         ELSE
             ! default 2D field rn_pfac2d
             rn_pfac2d(:,:) = rn_pfac
         ENDIF
         !
         ! 2D multiplication factor for precipitation
         ALLOCATE( rn_vfac2d(jpi,jpj), STAT=ierror )
         IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'sbc_blk_core: unable to allocate rn_vfac2d' )
         rn_vfac2d(:,:) = 0._wp
         !
         IF ( ln_vfac2d ) THEN
            WRITE(cl_vfacfile,'(a,a)' ) TRIM( cn_dir ), TRIM( sn_vfac%clname )
            CALL iom_open ( cl_vfacfile, inum )! open file
            CALL iom_get  ( inum, jpdom_data, sn_vfac%clvar, rn_vfac2d )! read the vfac array
            CALL iom_close( inum )
         ELSE
             ! default 2D field rn_vfac2d
             rn_vfac2d(:,:) = rn_vfac
         ENDIF
         !
         IF ( ln_kata ) THEN
	    ALLOCATE( rmskkatax(jpi,jpj), rmskkatay(jpi,jpj),  STAT=ierror )
	    !
	    !IF( lk_mpp            )   CALL mpp_sum ( sbc_kata_alloc )
	    IF( ierror > 0 )   CALL ctl_warn('sbc_kata_alloc: allocation of arrays failed')
	    rmskkatax(:,:) = 0._wp
	    rmskkatay(:,:) = 0._wp
	    !
...
!:MMS
      ...
      !{ MRA : using filtered sst for heat flux 
      IF ( MOD( kt - 1, nn_fsbc ) == 0 ) THEN
         sst_lp(:,:) = 0.e0
         ssu_lp(:,:) = 0.e0
         ssv_lp(:,:) = 0.e0

         IF ( ln_sst_flt .AND. ln_stress_flt) THEN             ! Thermal & Mechanical
            CALL avg_2D ( sst_m(:,:),  'ORCA_GLOB', sst_lp )
            CALL avg_2D ( ssu_m(:,:),  'ORCA_GLOB', ssu_lp )
            CALL avg_2D ( ssv_m(:,:),  'ORCA_GLOB', ssv_lp )
            sst_lp = sst_lp * tmask(:,:,1)
            ssu_lp = ssu_lp * tmask(:,:,1)
            ssv_lp = ssv_lp * tmask(:,:,1)
            CALL blk_oce_core( sf, sst_lp, ssu_lp, ssv_lp )
         
         ELSEIF ( ln_sst_flt .AND. .NOT.ln_stress_flt  ) THEN  ! Thermal
            CALL avg_2D ( sst_m(:,:),  'ORCA_GLOB', sst_lp )
            sst_lp = sst_lp * tmask(:,:,1)
            CALL blk_oce_core( sf, sst_lp, ssu_m, ssv_m )
         ELSEIF ( ln_stress_flt .AND. .NOT. ln_sst_flt ) THEN  ! Mechanical
            CALL avg_2D ( ssu_m(:,:),  'ORCA_GLOB', ssu_lp )
            CALL avg_2D ( ssv_m(:,:),  'ORCA_GLOB', ssv_lp )
            ssu_lp = ssu_lp * tmask(:,:,1)
            ssv_lp = ssv_lp * tmask(:,:,1)
            CALL blk_oce_core( sf, sst_m, ssu_lp, ssv_lp )
         ELSE                                                  ! No Filter 
         ...
         CALL iom_put( "sst_lp"     ,  sst_lp)
         CALL iom_put( "ssu_lp"     ,  ssu_lp)
         CALL iom_put( "ssv_lp"     ,  ssv_lp)

      ENDIF
      !}

      !MRA IF( MOD( kt - 1, nn_fsbc ) == 0 )   CALL blk_oce_core( sf, sst_m, ssu_m, ssv_m )
   ...
   SUBROUTINE blk_oce_core( sf, pst, pu, pv )
      ...
      REAL(wp), DIMENSION(:,:), POINTER ::   zwnd_i_rel, zwnd_j_rel    ! wind speed components at T-point MRA
      REAL(wp), DIMENSION(:,:), POINTER ::   zwnd_i_abs, zwnd_j_abs    ! wind speed components at T-point MRA
      !REAL(wp), DIMENSION(:,:), POINTER ::   wndm_rel, wndm_abs    ! wind speed components at T-point MRA
      ...
      CALL wrk_alloc( jpi,jpj, zwnd_i_rel, zwnd_j_rel, zwnd_i_abs, zwnd_j_abs)!, wndm_abs, wndm_rel )
      !
...
!CDIR COLLAPSE
  ...
  DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vect. opt.
            zwnd_i(ji,jj) = (  sf(jp_wndi)%fnow(ji,jj,1) - rn_vfac2d(ji,jj) * 0.5 * (pu(ji-1,jj  ) + pu(ji,jj) )  )
            zwnd_j(ji,jj) = (  sf(jp_wndj)%fnow(ji,jj,1) - rn_vfac2d(ji,jj) * 0.5 * ( pv(ji,jj-1) + pv(ji,jj) )  )
         END DO
      END DO
...
!CDIR COLLAPSE
      ...
      !MRA modified _rel added
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vect. opt.
            zwnd_i_rel(ji,jj) = (  sf(jp_wndi)%fnow(ji,jj,1) - rn_vfac2d(ji,jj) * 0.5 * ( pu(ji-1,jj  ) + pu(ji,jj) )  )
            zwnd_j_rel(ji,jj) = (  sf(jp_wndj)%fnow(ji,jj,1) - rn_vfac2d(ji,jj) * 0.5 * ( pv(ji  ,jj-1) + pv(ji,jj) )  )
         END DO
      END DO
      CALL lbc_lnk( zwnd_i_rel(:,:) , 'T', -1. )
      CALL lbc_lnk( zwnd_j_rel(:,:) , 'T', -1. )

      ! ... scalar wind ( = | U10m - U_oce | ) at T-point (masked)
!CDIR NOVERRCHK
!CDIR COLLAPSE
      wndm_rel(:,:) = SQRT(  zwnd_i_rel(:,:) * zwnd_i_rel(:,:)   &
         &             + zwnd_j_rel(:,:) * zwnd_j_rel(:,:)  ) * tmask(:,:,1)

  !MRA again without ocean velocity
       DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vect. opt.
            zwnd_i_abs(ji,jj) = (  sf(jp_wndi)%fnow(ji,jj,1) )! - 0.5 * ( pu(ji-1,jj  )+ pu(ji,jj) )  )
            zwnd_j_abs(ji,jj) = (  sf(jp_wndj)%fnow(ji,jj,1) )! - 0.5 * ( pv(ji  ,jj-1)+ pv(ji,jj) )  )
         END DO
      END DO
      CALL lbc_lnk( zwnd_i_abs(:,:) , 'T', -1. )
      CALL lbc_lnk( zwnd_j_abs(:,:) , 'T', -1. )
      CALL iom_put( "wind_u_rel" ,  zwnd_i_rel) !MRA
      CALL iom_put( "wind_v_rel" ,  zwnd_j_rel) !MRA
      CALL iom_put( "wind_u"     ,  zwnd_i_abs) !MRA
      CALL iom_put( "wind_v"     ,  zwnd_j_abs) !MRA
      t_a(:,:)= sf(jp_tair)%fnow(:,:,1)
      q_a(:,:)= sf(jp_humi)%fnow(:,:,1)
      CALL iom_put( "t_a"        ,  t_a) !MRA
      CALL iom_put( "q_a"        ,  q_a) !MRA
      ! ... scalar wind ( = | U10m - U_oce | ) at T-point (masked)
!CDIR NOVERRCHK
!CDIR COLLAPSE
      wndm_abs(:,:) = SQRT(  zwnd_i_abs(:,:) * zwnd_i_abs(:,:)   &
         &             + zwnd_j_abs(:,:) * zwnd_j_abs(:,:)  ) * tmask(:,:,1)
  !/MRA 
      ! ----------------------------------------------------------------------------- !
      !      I   Radiative FLUXES                                                     !
      ! ----------------------------------------------------------------------------- !
...
!CDIR COLLAPSE
      ...
      !MRA wind work output
   !   DO jj = 2, jpjm1
   !      DO ji = fs_2, fs_jpim1   ! vect. opt.
   !         u_at_t(ji,jj) = (   0.5 * (pu(ji-1,jj  ) + pu(ji,jj) )  )
   !         v_at_t(ji,jj) = (   0.5 * ( pv(ji,jj-1)  + pv(ji,jj) )  )
   !      END DO
   !   END DO
   !   CALL lbc_lnk( u_at_t(:,:) , 'T', -1. )
   !   CALL lbc_lnk( v_at_t(:,:) , 'T', -1. )
   ! 
   !   CALL iom_put( "wind_work", zwnd_i * u_at_t + zwnd_j * v_at_t )   ! output wind stress module
...
!MMS:{
      ...
      !
      IF (ln_rainplussnow) THEN
      ! This assumes that rain and snow are added to one file (NEMO-default)
!CDIR COLLAPSE
          emp (:,:) = zevap(:,:) - sf(jp_prec)%fnow(:,:,1) * rn_pfac2d(:,:) * tmask(:,:,1)
      ELSE
      ! Since we keep rain and snow in separate files we have to add here.
!CDIR COLLAPSE
          emp (:,:) = zevap(:,:) - ( sf(jp_prec)%fnow(:,:,1) + sf(jp_snow)%fnow(:,:,1) ) * rn_pfac2d(:,:) * tmask(:,:,1)
      ENDIF
!!
...
!CDIR COLLAPSE
      ...
      CALL iom_put( "Cd",   Cd  )                 ! output downward non solar heat over the ocean
      !
      IF(ln_ctl) THEN
      ...
      ENDIF
      !
      CALL wrk_dealloc( jpi,jpj, zwnd_i_rel, zwnd_j_rel,zwnd_i_abs, zwnd_j_abs) !, wndm_abs, wndm_rel )
...
!MMS:{
      ...
      IF (ln_rainplussnow) THEN
         ! This assumes that rain and snow are added to one file as total precipitation
!CDIR COLLAPSE
         p_tpr(:,:) = sf(jp_prec)%fnow(:,:,1) * rn_pfac2d(:,:)      ! total precipitation [kg/m2/s]
      ELSE
         ! Since we keep rain and snow in separate files we have to add here to get total precipitation.
!CDIR COLLAPSE
         p_tpr(:,:) = (sf(jp_prec)%fnow(:,:,1) + sf(jp_snow)%fnow(:,:,1) ) * rn_pfac2d(:,:)      ! total precipitation [kg/m2/s]
      ENDIF

!:MMS}
!CDIR COLLAPSE
      p_spr(:,:) = sf(jp_snow)%fnow(:,:,1) * rn_pfac2d(:,:)      ! solid precipitation [kg/m2/s]
