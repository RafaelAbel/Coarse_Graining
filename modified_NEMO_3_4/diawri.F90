...
# if defined key_iomput
   ...
   SUBROUTINE dia_wri( kt )
      ...
      !MRA
      CALL iom_put( "u2"     , un * un                               )    !i-current   u2   
      CALL iom_put( "v2"     , vn * vn                               )    !j-current   v2

      !MRA
      !/MRA 

      ! v * T, interpolate T at v point
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vect. opt.
            T_at_v(ji,jj,:) = (   0.5 * (tsn(ji,jj-1,:,jp_tem)  + tsn(ji,jj,:,jp_tem) )  )
            T_at_u(ji,jj,:) = (   0.5 * (tsn(ji-1,jj,:,jp_tem)  + tsn(ji,jj,:,jp_tem) )  ) 
            S_at_v(ji,jj,:) = (   0.5 * (tsn(ji,jj-1,:,jp_sal)  + tsn(ji,jj,:,jp_sal) )  )
            S_at_u(ji,jj,:) = (   0.5 * (tsn(ji-1,jj,:,jp_sal)  + tsn(ji,jj,:,jp_sal) )  )
         END DO
      END DO
      !WRITE(numout,*) 'T_at_v:',T_at_v
      !WRITE(numout,*) 'NEXT'
      v_T(:,:,:)=T_at_v(:,:,:)  * vn(:,:,:)
      u_T(:,:,:)=T_at_u(:,:,:)  * un(:,:,:)
      v_S(:,:,:)=S_at_v(:,:,:)  * vn(:,:,:)
      ...
      CALL iom_put( "v_T"     ,  v_T)
      CALL iom_put( "u_T"     ,  u_T)
      CALL iom_put( "v_S"     ,  v_S)
      CALL iom_put( "u_S"     ,  u_S)
      !/MRA
 
      !CALL iom_put( "wind_work_u"     , utau * un(:,:,1))! Wind work at U-point
      !CALL iom_put( "wind_work_v"     , vtau * vn(:,:,1))! Wind work at U-point
      !CALL iom_put( "wind_work_v"     , un )! Wind work at U-point
      !/MRA 
...
!MMS  CHARACTER (len=40)  ::   clhstnam, clop, clmx           ! local names
      CHARACTER (len=100) ::   clhstnam, clop, clmx           ! local names
